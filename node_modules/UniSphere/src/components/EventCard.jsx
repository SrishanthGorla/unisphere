...
// Placeholder functions for event sharing (since eventUtils is missing)
const generateGoogleCalendarLink = (event) => {
  // Simple placeholder - in a real app this would generate a proper Google Calendar link
  return `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(event.title)}&dates=${event.date ? new Date(event.date).toISOString().replace(/[-:]/g, '').split('.')[0] + 'Z' : ''}&details=${encodeURIComponent(event.description)}&location=${encodeURIComponent(event.venue)}`;
};

const shareEventOnSocial = (event, platform) => {
