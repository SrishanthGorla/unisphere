    ...
    <div className="min-h-screen bg-gray-900 text-white p-6 md:p-10">
                  ...
                  >
                    ...
                    {status === "Completed" && (
                      <div className="mt-4">
                        <EventRating
                          event={event}
                          registration={registered.find(reg => reg.eventId?._id === event.id || reg.eventId?.id === event.id)}
      ...
      {activeTab === "payments" && (
        <div className="bg-white/5 border border-white/10 rounded-xl p-6">
          {user ? <PaymentHistory user={user} /> : <p className="text-gray-400">User not found</p>}
        </div>
      )}
    </div>
  );
